...
- Never invent availability, prices, services, or opening hours. Every fact
  you state to the customer must come from a script's JSON output in this
  conversation.
...
1. **Identify the customer.** Run:
   ...
   - If the customer is new (`"created": true` in the response) and you don't
     have their name yet, ask for it first, then re-run with `--name`.
   - Keep the returned `customer_id` for the rest of the conversation.

2. **Understand the request.** Work out which service (from the business's
   known services - ask if ambiguous) and roughly which date/time window
   (e.g. "this Friday afternoon") the customer wants. Do not assume a
   service or duration that wasn't stated or confirmed.

3. **Check availability.** Run:
...
4. **Confirm a choice.** Once the customer picks one of the *exact* slots you
   just presented, move to booking. If they propose a time you didn't offer,
   go back to step 3 for that date/time instead of accepting it directly.

5. **Create the booking.** Run:
   ...
   - This script rechecks availability itself immediately before booking -
     time may have passed since step 3. If it returns a conflict
     (`"error": "slot_unavailable"`), tell the customer that slot was just
     taken, and go back to step 3 for fresh options. Do not retry the same
     slot.
   ...
   - On success, confirm with the exact service, date, time and business
     name from the script's response.

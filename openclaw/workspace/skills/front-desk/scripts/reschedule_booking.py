#!/usr/bin/env python3
"""reschedule_booking.py --business <business_id> --booking_id <uuid> --customer_id <uuid> --new_start 2026-09-27T14:00:00+08:00 --json

Moves an existing confirmed booking to a new time (same service, same
booking id - not a new booking). --business must be this session's own
business - the backend rejects a booking that belongs to a different
business even for the same customer. Rechecks availability itself
immediately before moving it. Only report success if this returns
"status": "confirmed" - any other outcome, including "slot_unavailable",
means nothing changed and the original booking is still in effect at its
original time.
"""
...
def main() -> None:
    ...
    payload = {"customer_id": args.customer_id, "business_id": args.business_id, "new_start": args.new_start}
